...
const BASIC_ENEMY_SETTIMGS = \
preload("res://resources/basic_enemy_settings.res")
const POWER_ENEMY_SETTIMGS = \
preload("res://resources/power_enemy_settings.res")
...
func _spawn_wave():	
	...
	= enemy_waves[current_wave_index].enemies
	wave_spawned = false
	enemies_remaining = 0
		
	for i in range(enemy_wave.size()):
		await get_tree().create_timer(enemy_wave[i]\
		.next_enemy_delay).timeout
